...
import { getCompanyViewList } from '../../../services/transportcompany.service'
...
const SBUCompnayGrid: React.FC<{
  ...
  const CompanyViewList=useSelector((state: any) => state.transportCompany.CompanyViewList);
  const CompanyViewListArray = CompanyViewList.map((branch: any) => branch);
  console.log('CompanyViewListArray', CompanyViewListArray);
  ...
  useEffect(() => {
    ...
    store.dispatch(getCompanyViewList());
  ...
  <AppSkeleton numOfRows={APP_TABLE_CONFIGS.DEFAULT_ROWS_PER_PAGE} numOfColumns={21} columnWidth={80} tag="table" />
          ...
          {!props.approvalRequestDataIsLoading && CompanyViewListArray.length > 0 && 
  <TableBody>
    {CompanyViewListArray.slice(props.page * props.rowsPerPage, props.page * props.rowsPerPage + props.rowsPerPage).map((req: ApprovalCompanyListDto) => (
                <TableRow key={req.companyId}>
                  ...
                  <StyledTableCell>
                      {req.sbu.map((item) => item.name).join(', ')}
                  </StyledTableCell>
                  <StyledTableCell>{req.status ? 'Active' : 'Inactive'}</StyledTableCell>
