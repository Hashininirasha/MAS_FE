...
import { APP_ACTION_STATUS, APP_TABLE_CONFIGS, COMPANY_REQUEST_STATUS } from '../../utilities/constants'
...
const CompanyRequest = () => {
  ...
  const setSummaryCount = () => {
    ...
    getRequestDataResponse.data.forEach((r: RequestListDto) => {
      if (r.status === COMPANY_REQUEST_STATUS.ACTIVE || r.status === COMPANY_REQUEST_STATUS.ACTIVE) summary.approved++
      else if (r.status === COMPANY_REQUEST_STATUS.ACTIVE || r.status === COMPANY_REQUEST_STATUS.ACTIVE) summary.reject++
      else if (r.status === COMPANY_REQUEST_STATUS.INACTIVE) summary.pending++
    })
